...
Get access to some PostgreSQL instance (version 11 is supported, but versions 9 and higher should work).
...
```
>>> import pg_jts
>>> jts, notifications = pg_jts.get_database('host=127.0.0.1 user=testuser dbname=testdb port=5432 password=***************')
```
